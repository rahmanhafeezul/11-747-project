...
def load_mednli(file, label_dict, header=True): # header argument specifies is header is present in the file
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
            if header:
                # To skip the first line which is the header
                header = False
                continue
            blocks = line.strip().split('\t')
            try:
            ...
            lab = label_dict[blocks[-1]] # assuming last column is the label
            if lab is None:
                import pdb; pdb.set_trace()
            lab = 0 if lab is None else lab
            sample = {'uid': cnt, 'premise': blocks[0], 'hypothesis': blocks[3], 'label': lab}
            rows.append(sample)
            cnt += 1
    return rows

def load_rqe(file, label_dict, header=True): # header argument specifies is header is present in the file
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
            ...
            if lab is None:
                import pdb; pdb.set_trace()
            lab = 0 if lab is None else lab
            sample = {'uid': blocks[0], 'premise': blocks[2], 'hypothesis': blocks[3], 'label': lab}
            rows.append(sample)
            cnt += 1
    return rows
...
def load_yelp(file, header=False, is_train=True):
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
...
def load_imdb(file, header=False, is_train=True):
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
...
def load_nnlp(file, header=False, is_train=True):
    rows = []
    cnt = 0
    data = pandas.read_csv(file, encoding = "ISO-8859-1")
    if(is_train):
        print("Parsing NNLP training data or val data")
        uid_list = data["ID"].tolist()
        text_list = data["Text"].tolist()
        label_list = data["Label"].tolist()

        for i in range(len(uid_list)):
            sample = {'uid': uid_list[i], 'premise': text_list[i], 'label': int(label_list[i])}
            rows.append(sample)

    else:
        print("Parsing NNLP test data")
        uid_list = data["ID"].tolist()
        text_list = data["Text"].tolist()

        for i in range(len(uid_list)):
            sample = {'uid': uid_list[i], 'premise': text_list[i], 'label': 0}
            rows.append(sample)
