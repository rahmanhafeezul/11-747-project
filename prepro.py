...
def build_data_single(data, dump_path, max_seq_len=MAX_SEQ_LEN):
    ...
    with open(dump_path, 'w', encoding='utf-8') as writer:
        for idx, sample in enumerate(tqdm.tqdm(data)):
            ids = sample['uid']
            premise = bert_tokenizer.tokenize(sample['premise'])
            label = sample['label']
...
def parse_args():
    parser = argparse.ArgumentParser(description='Preprocessing NNLP datasets.')
...
def main(args):
    ...
    ######################################
    # ACL BioNLP 2019 Tasks
    ######################################
    
    # mednli_train_path = os.path.join(root, "MEDNLI/MEDNLI_train.tsv")
    # mednli_dev_path = os.path.join(root, "MEDNLI/MEDNLI_dev.tsv")
    # mednli_test_path = os.path.join(root, "MEDNLI/MEDNLI_test.tsv")

    # rqe_train_path = os.path.join(root, "RQE/RQE_train.tsv")
    # rqe_test_path = os.path.join(root, "RQE/RQE_test.tsv")



    yelp_train_path = os.path.join(root, 'yelp_data/yelp_train.tsv')
    yelp_dev_path = os.path.join(root, 'yelp_data/yelp_dev.tsv')
    yelp_test_path = os.path.join(root, 'yelp_data/yelp_test.tsv')

    imdb_train_path = os.path.join(root, 'imdb_data/imdb_train.tsv')
    imdb_dev_path = os.path.join(root, 'imdb_data/imdb_dev.tsv')
    imdb_test_path = os.path.join(root, 'imdb_data/imdb_test.tsv')


    suggestion_train_path = os.path.join(root, 'nnlp_data/Subtask-A/V1.4_Training.csv')
    suggestion_dev_path = os.path.join(root, 'nnlp_data/Subtask-A/SubtaskA_Trial_Test_Labeled.csv')
    suggestion_test_path = os.path.join(root, 'nnlp_data/Subtask-A/SubtaskA_EvaluationData.csv')
    ...
    yelp_test_data = load_yelp(yelp_test_path, is_train=False)
    logger.info('Loaded {} Yelp train samples'.format(len(yelp_train_data)))
    logger.info('Loaded {} Yelp dev samples'.format(len(yelp_dev_data)))
    logger.info('Loaded {} Yelp test samples'.format(len(yelp_test_data)))

    imdb_train_data = load_imdb(imdb_train_path)
    imdb_dev_data = load_imdb(imdb_dev_path)
    imdb_test_data = load_imdb(imdb_test_path, is_train=False)
    ...
    logger.info('Loaded {} IMDB test samples'.format(len(imdb_test_data)))

    nnlp_train_data = load_nnlp(suggestion_train_path)
    nnlp_dev_data = load_nnlp(suggestion_dev_path)
    nnlp_test_data = load_nnlp(suggestion_test_path, is_train=False)
    logger.info('Loaded {} NNLP train samples'.format(len(nnlp_train_data)))
    logger.info('Loaded {} NNLP dev samples'.format(len(nnlp_dev_data)))
    logger.info('Loaded {} NNLP test samples'.format(len(nnlp_test_data)))
    ...
    pdb.set_trace()
    yelp_train_fout = os.path.join(mt_dnn_root, 'yelp_train.json')
    yelp_dev_fout = os.path.join(mt_dnn_root, 'yelp_dev.json')
    yelp_test_fout = os.path.join(mt_dnn_root, 'yelp_test.json')
    build_data_single(yelp_train_data, yelp_train_fout)
    build_data_single(yelp_dev_data, yelp_dev_fout)
    build_data_single(yelp_test_data, yelp_test_fout)
    logger.info('done with yelp')


    imdb_train_fout = os.path.join(mt_dnn_root, 'imdb_train.json')
    imdb_dev_fout = os.path.join(mt_dnn_root, 'imdb_dev.json')
    imdb_test_fout = os.path.join(mt_dnn_root, 'imdb_test.json')
    build_data_single(imdb_train_data, imdb_train_fout)
    build_data_single(imdb_dev_data, imdb_dev_fout)
    build_data_single(imdb_test_data, imdb_test_fout)
    logger.info('done with imdb')

    nnlp_train_fout = os.path.join(mt_dnn_root, 'nnlp_train.json')
    nnlp_dev_fout = os.path.join(mt_dnn_root, 'nnlp_dev.json')
    nnlp_test_fout = os.path.join(mt_dnn_root, 'nnlp_test.json')
    build_data_single(nnlp_train_data, nnlp_train_fout)
    build_data_single(nnlp_dev_data, nnlp_dev_fout)
    build_data_single(nnlp_test_data, nnlp_test_fout)
    logger.info('done with suggestion mining')
